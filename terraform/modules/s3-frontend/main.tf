...
resource "aws_s3_bucket" "frontend" {
  bucket = "${var.name_prefix}-frontend-dryrun"
  tags   = local.tags
}
...
# Upload env.js with backend URL configuration
# This file is loaded before app.js and sets the API endpoint
resource "aws_s3_object" "env" {
  bucket       = aws_s3_bucket.frontend.id
  key          = "env.js"
  content      = <<-EOF
// Auto-generated by Terraform - Backend API configuration
window.__API_BASE__ = "${var.backend_url}";
EOF
  content_type = "application/javascript"
  # Force update when backend_url changes
  etag = md5("${var.backend_url}")
